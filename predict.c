...
double tsince, jul_epoch, jul_utc,
    eclipse_depth = 0, sat_azi, sat_ele, sat_range, sat_range_rate, sat_lat,
    sat_lon, sat_alt, sat_vel, phase, sun_azi, sun_ele, daynum, fm, fk, age,
    aostime, lostime, ax, ay, az, rx, ry, rz, squint, alat, alon, sun_ra,
...
int isFlagSet(int flag) { return (Flags & flag); }

int isFlagClear(int flag) { return (~Flags & flag); }
...
void Calculate_Solar_Position(double time, vector_t *solar_vector) {
  ...
  M = Radians(Modulus(358.47583 + Modulus(35999.04975 * T, 360.0) -
                      ...
                      360.0));
  L = Radians(Modulus(
      279.69668 + Modulus(36000.76892 * T, 360.0) + 0.0003025 * Sqr(T), 360.0));
  ...
  eps =
      Radians(23.452294 - (0.0130125 + (0.00000164 - 0.000000503 * T) * T) * T +
              0.00256 * cos(O));
  R = AU * R;
...
void select_ephemeris(tle_t *tle) {
  ...
  del1 = temp / (a1 * a1);
  ao = a1 *
       (1.0 - del1 * (tothrd * .5 + del1 * (del1 * 1.654320987654321 + 1.0)));
  delo = temp / (ao * ao);
...
void SGP4(double tsince, tle_t *tle, vector_t *pos, vector_t *vel) {
  ...
  static double aodp, aycof, c1, c4, c5, cosio, d2, d3, d4, delmo, omgcof, eta,
      omgdot, sinio, xnodp, sinmo, t2cof, t3cof, t4cof, t5cof, x1mth2, x3thm1,
      x7thm1, xmcof, xmdot, xnodcf, xnodot, xlcof;

  double cosuk, sinuk, rfdotk, vx, vy, vz, ux, uy, uz, xmy, xmx, cosnok, sinnok,
      cosik, sinik, rdotk, xinck, xnodek, uk, rk, cos2u, sin2u, u, sinu, cosu,
      betal, rfdot, rdot, r, pl, elsq, esine, ecose, epw, cosepw, x1m5th,
      xhdot1, tfour, sinepw, capu, ayn, xlt, aynl, xll, axn, xn, beta, xl, e, a,
      tcube, delm, delomg, templ, tempe, tempa, xnode, tsq, xmp, omega, xnoddf,
      omgadf, xmdf, a1, a3ovk2, ao, betao, betao2, c1sq, c2, c3, coef, coef1,
      del1, delo, eeta, eosq, etasq, perigee, pinvsq, psisq, qoms24, s4, temp,
      temp1, temp2, temp3, temp4, temp5, temp6, theta2, theta4, tsi;
  ...
  if (isFlagClear(SGP4_INITIALIZED_FLAG)) {
    ...
    ao =
        a1 * (1.0 - del1 * (0.5 * tothrd + del1 * (1.0 + 134.0 / 81.0 * del1)));
    ...
    xhdot1 = -temp1 * cosio;
    xnodot = xhdot1 +
             (0.5 * temp2 * (4 - 19 * theta2) + 2 * temp3 * (3 - 7 * theta2)) *
                 cosio;
...
void Deep(int ientry, tle_t *tle, deep_arg_t *deep_arg) {
  ...
  static double thgr, xnq, xqncl, omegaq, zmol, zmos, savtsn, ee2, e3, xi2, xl2,
      xl3, xl4, xgh2, xgh3, xgh4, xh2, xh3, sse, ssi, ssg, xi3, se2, si2, sl2,
      sgh2, sh2, se3, si3, sl3, sgh3, sh3, sl4, sgh4, ssl, ssh, d3210, d3222,
      d4410, d4422, d5220, d5232, d5421, d5433, del1, del2, del3, fasx2, fasx4,
      fasx6, xlamo, xfact, xni, atime, stepp, stepn, step2, preep, pl, sghs,
      xli, d2201, d2211, sghl, sh1, pinc, pe, shs, zsingl, zcosgl, zsinhl,
      zcoshl, zsinil, zcosil;

  double a1, a2, a3, a4, a5, a6, a7, a8, a9, a10, ainv2, alfdp, aqnv, sgh,
      sini2, sinis, sinok, sh, si, sil, day, betdp, dalf, bfact, c, cc, cosis,
      cosok, cosq, ctem, f322, zx, zy, dbet, dls, eoc, eq, f2, f220, f221, f3,
      f311, f321, xnoh, f330, f441, f442, f522, f523, f542, f543, g200, g201,
      g211, pgh, ph, s1, s2, s3, s4, s5, s6, s7, se, sel, ses, xls, g300, g310,
      g322, g410, g422, g520, g521, g532, g533, gam, sinq, sinzf, sis, sl, sll,
      sls, stem, temp, temp1, x1, x2, x2li, x2omi, x3, x4, x5, x6, x7, x8, xl,
      xldot, xmao, xnddt, xndot, xno2, xnodce, xnoi, xomi, xpidot, z1, z11, z12,
      z13, z2, z21, z22, z23, z3, z31, z32, z33, ze, zf, zm, zn, zsing, zsinh,
  ...
  case dpinit: /* Entrance for deep space initialization */
    ...
    for (;;) {
      ...
      z22 =
          6 * (a4 * a5 + a2 * a6) +
          deep_arg->eosq * (24 * (x2 * x5 + x1 * x6) - 6 * (x4 * x7 + x3 * x8));
    ...
    if (!((xnq < 0.0052359877) && (xnq > 0.0034906585))) {
      ...
      if (eq <= 0.65) {
        ...
        g310 =
            -19.302 + 117.390 * eq - 228.419 * deep_arg->eosq + 156.591 * eoc;
        g322 = -18.9068 + 109.7927 * eq - 214.6334 * deep_arg->eosq +
               146.5816 * eoc;
        g410 =
            -41.122 + 242.694 * eq - 471.094 * deep_arg->eosq + 313.953 * eoc;
        g422 = -146.407 + 841.880 * eq - 1629.014 * deep_arg->eosq +
               1083.435 * eoc;
        g520 =
            -532.114 + 3017.977 * eq - 5740 * deep_arg->eosq + 3708.276 * eoc;
      }

      else {
        g211 =
            -72.099 + 331.819 * eq - 508.738 * deep_arg->eosq + 266.724 * eoc;
      ...
      if (eq < 0.7) {
        g533 =
            -919.2277 + 4988.61 * eq - 9064.77 * deep_arg->eosq + 5542.21 * eoc;
        g521 = -822.71072 + 4568.6173 * eq - 8491.4146 * deep_arg->eosq +
               5337.524 * eoc;
        g532 =
            -853.666 + 4690.25 * eq - 8624.77 * deep_arg->eosq + 5341.4 * eoc;
      }
      ...
      bfact =
          deep_arg->xmdot + deep_arg->xnodot + deep_arg->xnodot - thdt - thdt;
  ...
  case dpper: /* Entrance for lunar-solar periodics */
    ...
    if (xqncl >= 0.2) {
      ...
      deep_arg->xll = deep_arg->xll + pl;
      deep_arg->omgadf =
          xls - deep_arg->xll - cos(deep_arg->xinc) * deep_arg->xnode;
    }
    return;
  }
}
...
void SDP4(double tsince, tle_t *tle, vector_t *pos, vector_t *vel) {
  ...
  double a, axn, ayn, aynl, beta, betal, capu, cos2u, cosepw, cosik, cosnok,
      cosu, cosuk, ecose, elsq, epw, esine, pl, theta4, rdot, rdotk, rfdot,
      rfdotk, rk, sin2u, sinepw, sinik, sinnok, sinu, sinuk, tempe, templ, tsq,
      u, uk, ux, uy, uz, vx, vy, vz, xinck, xl, xlt, xmam, xmdf, xmx, xmy,
      xnoddf, xnodek, xll, a1, a3ovk2, ao, c2, coef, coef1, x1m5th, xhdot1,
      del1, r, delo, eeta, eta, etasq, perigee, psisq, tsi, qoms24, s4, pinvsq,
      temp, tempa, temp1, temp2, temp3, temp4, temp5, temp6, bx, by, bz, cx, cy,
      cz;
  ...
  if (isFlagClear(SDP4_INITIALIZED_FLAG)) {
    ...
    pinvsq =
        1 / (deep_arg.aodp * deep_arg.aodp * deep_arg.betao2 * deep_arg.betao2);
...
void Calculate_RADec(double time, vector_t *pos, vector_t *vel,
  ...
  double phi, theta, sin_theta, cos_theta, sin_phi, cos_phi, az, el, Lxh, Lyh,
      Lzh, Sx, Ex, Zx, Sy, Ey, Zy, Sz, Ez, Zz, Lx, Ly, Lz, cos_delta, sin_alpha,
      cos_alpha;
...
void bailout(string) char *string;
{
...
void TrackDataOut(antfd, elevation, azimuth) int antfd;
...
void socket_server(predict_name) char *predict_name;
{
  ...
  while (1) {
    ...
    if ((n = recvfrom(sock, buf, sizeof(buf), 0, (struct sockaddr *)&fsin,
                      &alen)) < 0)
      exit(-1);
    ...
    if ((strncmp("GET_SAT", buf, 7) == 0) &&
      ...
      for (i = 0; i < 24; i++) {
        if ((strncmp(satname, sat[i].name, 25) == 0) ||
            (atol(satname) == sat[i].catnum)) {
          ...
          sprintf(buff,
                  "%s\n%-7.2f\n%+-6.2f\n%-7.2f\n%+-6.2f\n%ld\n%-7.2f\n%-7.2f\n%"
                  "-7.2f\n%-7.2f\n%ld\n%c\n%-7.2f\n%-7.2f\n%-7.2f\n",
                  sat[i].name, long_array[i], lat_array[i], az_array[i],
                  el_array[i], nxtevt, footprint_array[i], range_array[i],
                  altitude_array[i], velocity_array[i], orbitnum_array[i],
                  visibility_array[i], phase_array[i], eclipse_depth_array[i],
                  squint_array[i]);

          /* Send buffer back to the client that sent the request */
          sendto(sock, buff, strlen(buff), 0, (struct sockaddr *)&fsin,
                 sizeof(fsin));
          ok = 1;
          break;
        }
    ...
    if (strncmp("GET_SUN", buf, 7) == 0) {
      buff[0] = 0;
      sprintf(buff, "%-7.2f\n%+-6.2f\n%-7.2f\n%-7.2f\n%-7.2f\n", sun_azi,
              sun_ele, sun_lat, sun_lon, sun_ra);
      sendto(sock, buff, strlen(buff), 0, (struct sockaddr *)&fsin,
             sizeof(fsin));
      ok = 1;
    }
    ...
    if (strncmp("GET_MOON", buf, 8) == 0) {
      buff[0] = 0;
      sprintf(buff, "%-7.2f\n%+-6.2f\n%-7.2f\n%-7.2f\n%-7.2f\n", moon_az,
              moon_el, moon_dec, moon_gha, moon_ra);
      sendto(sock, buff, strlen(buff), 0, (struct sockaddr *)&fsin,
             sizeof(fsin));
      ok = 1;
    }
    ...
    if (strncmp("GET_MODE", buf, 8) == 0) {
      sendto(sock, tracking_mode, strlen(tracking_mode), 0,
    ...
    if (strncmp("GET_QTH", buf, 7) == 0) {
      buff[0] = 0;
      sprintf(buff, "%s\n%g\n%g\n%d\n", qth.callsign, qth.stnlat, qth.stnlong,
              qth.stnalt);
      sendto(sock, buff, strlen(buff), 0, (struct sockaddr *)&fsin,
             sizeof(fsin));
      ok = 1;
    ...
    if (strncmp("GET_SAT_POS", buf, 11) == 0) {
      ...
      sprintf(buff, "%s -f %s -t %s -q %s -o %s\n", predict_name, satname,
              tlefile, qthfile, tempname);
      system(buff);
...
char *SubString(string, start, end) char *string, start, end;
{
...
void CopyString(source, destination, start, end) char *source,
...
char *Abbreviate(string, n) char *string;
int n;
{
...
char KepCheck(line1, line2) char *line1, *line2;
{
...
char *noradEvalue(value) double value;
{
...
double ReadBearing(input) char *input;
{
  ...
  if (fd != NULL) {
    while (x < 24 && feof(fd) == 0) {
      ...
      if (KepCheck(line1, line2) && (feof(fd) == 0)) {
        ...
        while (name[y] == 32 || name[y] == 0 || name[y] == 10 ||
               name[y] == 13 || y == 0) {
          name[y] = 0;
          y--;
        }
      ...
      while (strncmp(line1, "end", 3) != 0 && line1[0] != '\n' &&
        ...
        while (strncmp(line1, "end", 3) != 0 && line1[0] != '\n' &&
          ...
          if (entry < max_entries) {
            ...
            if (match)
              sscanf(line1, "%lf, %lf", &sat_db[y].uplink_start[entry],
                     &sat_db[y].uplink_end[entry]);

            fgets(line1, 40, fd);

            if (match)
              sscanf(line1, "%lf, %lf", &sat_db[y].downlink_start[entry],
                     &sat_db[y].downlink_end[entry]);
            ...
            if (match) {
              if (strncmp(line1, "No", 2) != 0)
                sscanf(line1, "%d, %d", &sat_db[y].phase_start[entry],
                       &sat_db[y].phase_end[entry]);
              else {
                sat_db[y].phase_start[entry] = 0;
                sat_db[y].phase_end[entry] = 0;
              }

              if (sat_db[y].uplink_start[entry] != 0.0 ||
                  sat_db[y].downlink_start[entry] != 0.0)
                transponders++;
...
char CopyFile(source, destination) char *source;
char *destination;
{
...
int AutoUpdate(string) char *string;
{
  ...
  char line1[80], line2[80], str0[80], str1[80], str2[80], filename[50],
      saveflag = 0, interactive = 0, savecount = 0;
  ...
  do {
    ...
    if (strlen(filename) == 0 || strncmp(filename, ".", 1) == 0 ||
    ...
    if (fd != NULL) {
      ...
      do {
        if (KepCheck(str1, str2)) {
          ...
          for (i = 0; (i < 24 && sat[i].catnum != atol(SubString(line1, 2, 6)));
               i++)
            ;
          ...
          if (i != 24) {
            ...
            tle_epoch =
                (float)atof(SubString(line1, 20, 31)) + (tle_year * 365.25);
        ...
        if (saveflag) {
          if (savecount == 1)
            ...
            if (savecount == 24)
              ...
              mvprintw(19, 21, "%3u out of 24 satellites were updated.",
                       savecount);
          }
        }
...
char *Daynum2String(daynum) double daynum;
{
...
double GetStartTime(mode) char mode;
{
  ...
  char good, mon[5], line[30], string[30],
      bozo_count = 0, *month[12] = {"Jan", "Feb", "Mar", "Apr", "May", "Jun",
  ...
  do {
    ...
    if (mode == 'm')
      printw("\n\n\n\t     Starting UTC Date and Time for Predictions of the "
             "Moon\n\n");
    ...
    if (mode != 'm' && mode != 'o')
      printw("\n\n\n\t     Starting UTC Date and Time for Predictions of "
             "%-15s\n\n",
             sat[indx].name);
    ...
    /* Month */
    good = (good && isalpha(line[2]) && isalpha(line[3]) && isalpha(line[4]))
               ? 1
               : 0;

    /* Year */
    good = (good && isdigit(line[5]) && isdigit(line[6]) && (line[7] == ' '))
               ? 1
               : 0;

    /* Hour */
    good = (good && isdigit(line[8]) && isdigit(line[9]) && (line[10] == ':'))
               ? 1
               : 0;

    /* Minute */
    good = (good && isdigit(line[11]) && isdigit(line[12]) && (line[13] == ':'))
               ? 1
               : 0;
  ...
  return ((double)DayNum(mm, dd, yy) +
          ((hr / 24.0) + (min / 1440.0) + (sec / 86400.0)));
}

void FindMoon(daynum) double daynum;
{
  ...
  double jd, ss, t, t1, t2, t3, d, ff, l1, m, m1, ex, om, l, b, w1, w2, bt, p,
      lm, h, ra, dec, z, ob, n, el, az, teg, th, mm, dv;
  ...
  l = l + ex * 0.045874 * sin(2.0 * d - m) + ex * 0.041024 * sin(m1 - m) -
      0.034718 * sin(d);
  l = l - ex * 0.030465 * sin(m + m1) + 0.015326 * sin(2.0 * d - 2.0 * ff) -
      0.012528 * sin(2.0 * ff + m1);

  l = l - 0.01098 * sin(2.0 * ff - m1) + 0.010674 * sin(4.0 * d - m1) +
      0.010034 * sin(3.0 * m1);
  l = l + 0.008548 * sin(4.0 * d - 2.0 * m1) -
      ex * 0.00791 * sin(m - m1 + 2.0 * d) - ex * 0.006783 * sin(2.0 * d + m);
  ...
  l = l - 0.002349 * sin(m1 + d) + ex * ex * 0.002249 * sin(2.0 * d - 2.0 * m) -
  ...
  l = l + ex * ex * 0.000704 * sin(m1 - 2.0 * m - 2.0 * d) +
      ...
      ex * 0.000598 * sin(2.0 * d - m - 2.0 * ff) + 0.00055 * sin(m1 + 4.0 * d);
  ...
  b = 5.128189 * sin(ff) + 0.280606 * sin(m1 + ff) + 0.277693 * sin(m1 - ff) +
      0.173238 * sin(2.0 * d - ff);
  b = b + 0.055413 * sin(2.0 * d + ff - m1) +
      0.046272 * sin(2.0 * d - ff - m1) + 0.032573 * sin(2.0 * d + ff);

  b = b + 0.017198 * sin(2.0 * m1 + ff) +
      9.266999e-03 * sin(2.0 * d + m1 - ff) + 0.008823 * sin(2.0 * m1 - ff);
  b = b + ex * 0.008247 * sin(2.0 * d - m - ff) +
      0.004323 * sin(2.0 * d - ff - 2.0 * m1) + 0.0042 * sin(2.0 * d + ff + m1);
  ...
  b = b + 0.002072 * sin(2.0 * d - ff - m - m1) +
      ex * 0.001877 * sin(ff - m + m1) + 0.001828 * sin(4.0 * d - ff - m1);

  b = b - ex * 0.001803 * sin(ff + m) - 0.00175 * sin(3.0 * ff) +
      ex * 0.00157 * sin(m1 - m - ff) - 0.001487 * sin(ff + d) -
      ex * 0.001481 * sin(ff + m + m1) + ex * 0.001417 * sin(ff - m - m1) +
      ex * 0.00135 * sin(ff - m) + 0.00133 * sin(ff - d);
  ...
  p = p + 0.000173 * cos(3.0 * m1) + 0.000167 * cos(4.0 * d - m1) -
      ...
      0.000084 * cos(2.0 * m1 - 2.0 * d) - ex * 0.000083 * cos(2.0 * d + m) +
...
void FindSun(daynum) double daynum;
{
  ...
  Calculate_RADec(jul_utc, &solar_vector, &zero_vector, &obs_geodetic,
                  &solar_rad);
...
void Calc() {
  ...
  rv = (long)floor((tle.xno * xmnpda / twopi + age * tle.bstar * ae) * age +
                   tle.xmo / twopi) +
       tle.revnum;
...
double time;
{
  ...
  if (satepoch + ((16.666666 - sat[x].meanmo) / (10.0 * fabs(sat[x].drag))) <
      time)
    return 1;
  else
    return 0;
...
double FindAOS() {
  ...
  if (AosHappens(indx) && Geostationary(indx) == 0 &&
      Decayed(indx, daynum) == 0) {
    Calc();
...
double FindLOS() {
  ...
  if (Geostationary(indx) == 0 && AosHappens(indx) == 1 &&
      Decayed(indx, daynum) == 0) {
    Calc();
...
int Print(string, mode) char *string, mode;
{
  ...
  char type[20], spaces[80], head1[160], head2[70], head3[72],
      satellite_name[25];
  int key, ans = 0, l, x, t;
  ...
  if (string[0] == 0) {
    ...
    if (mode == 'm') {
      sprintf(head1, "\n                    %s's Orbit Calendar for the Moon",
              qth.callsign);
      strncpy(satellite_name, "Moon\0", 5);
    }

    if (mode == 'o') {
      sprintf(head1, "\n                    %s's Orbit Calendar for the Sun",
              qth.callsign);
    ...
    if (mode != 'm' && mode != 'o') {
      ...
      sprintf(head1, "\n%s%s's %s Calendar For %s", spaces, qth.callsign, type,
              satellite_name);

      if (mode == 's')
        sprintf(head2,
                "\n\t  Date     Mins/Day    Sun%c          Date      Mins/Day  "
                "  Sun%c\n",
                37, 37);
      else {
        if (calc_squint)
          ...
          sprintf(head2,
                  "\n\t   Date     Time    El   Az  Phase  %s   %s    Range  "
                  "Squint\n",
                  (io_lat == 'N' ? "LatN" : "LatS"),
                  (io_lon == 'W' ? "LonW" : "LonE"));
        else
...
int PrintVisible(string) char *string;
{
...
void Predict(mode) char mode;
{
  ...
  if (AosHappens(indx) && Geostationary(indx) == 0 &&
    ...
    if (xterm) {
      ...
      fprintf(stderr, "\033]0;PREDICT: %s's %s Calendar For %s\007",
              qth.callsign, type, sat[indx].name);
    }
    ...
    } while (quit == 0 && breakout == 0 && AosHappens(indx) &&
             Decayed(indx, daynum) == 0);
  }
    ...
    if (AosHappens(indx) == 0 || Decayed(indx, daynum) == 1)
      mvprintw(12, 5,
               "*** Passes for %s cannot occur for your ground station! ***\n",
               sat[indx].name);

    if (Geostationary(indx) == 1)
      mvprintw(12, 3,
               "*** Orbital predictions cannot be made for a geostationary "
               "satellite! ***\n");
...
void PredictMoon() {
  ...
  if (xterm)
    fprintf(stderr, "\033]0;PREDICT: %s's Orbit Calendar for the Moon\007",
            qth.callsign);
  ...
  do {
    ...
    do {
      ...
      sprintf(string, "      %s%4d %4d  %5.1f  %5.1f  %5.1f  %6.1f%7.3f\n",
              Daynum2String(daynum), iel, iaz, moon_ra, moon_dec, moon_gha,
              moon_dv, moon_dx);
      quit = Print(string, 'm');
      lastel = iel;
      lastdaynum = daynum;
...
void PredictSun() {
  ...
  if (xterm)
    fprintf(stderr, "\033]0;PREDICT: %s's Orbit Calendar for the Sun\007",
            qth.callsign);
  ...
  do {
    ...
    do {
      ...
      sprintf(string, "      %s%4d %4d  %5.1f  %5.1f  %5.1f  %6.1f%7.3f\n",
              Daynum2String(daynum), iel, iaz, sun_ra, sun_dec, sun_lon,
              sun_range_rate, sun_range);
      quit = Print(string, 'o');
      lastel = iel;
      lastdaynum = daynum;
...
void ShowOrbitData() {
  ...
  while (x != -1) {
    if (sat[x].meanmo != 0.0) {
      ...
      no_period =
          (an_period * 360.0) / (360.0 + (4.97 * pow((xkmper / sma), 3.5) *
                                          ((5.0 * c1 * c1) - 1.0) / (e2 * e2)) /
                                             sat[x].meanmo);
      satepoch = DayNum(1, 0, sat[x].year) + sat[x].refepoch;
...
void SingleTrack(x, speak) int x;
char speak;
{
  ...
  int ans, oldaz = 0, oldel = 0, length, xponder = 0, polarity = 0, tshift,
           bshift;
  char approaching = 0, command[80], comsat, aos_alarm = 0, geostationary = 0,
       aoshappens = 0, decayed = 0, eclipse_alarm = 0, visibility = 0,
       old_visibility = 0;
  double oldtime = 0.0, nextaos = 0.0, lostime = 0.0, aoslos = 0.0,
         downlink = 0.0, uplink = 0.0, downlink_start = 0.0, downlink_end = 0.0,
         uplink_start = 0.0, uplink_end = 0.0, dopp, doppler100 = 0.0, delay,
         loss, shift;
  ...
  printw("                     PREDICT Real-Time Satellite Tracking            "
         "            ");
  printw("                 Tracking: %-10sOn                                   "
         "    ",
         Abbreviate(sat[x].name, 9));
  printw("                                                                     "
         "            ");
  ...
  mvprintw(5 + tshift, 1,
           "Satellite     Direction     Velocity     Footprint    Altitude     "
           "Slant Range");
  mvprintw(6 + tshift, 1,
           "---------     ---------     --------     ---------    --------     "
           "-----------");
  ...
  mvprintw(8 + tshift, 1,
           "        .            El           km            km          km     "
           "         km");
  mvprintw(16 + bshift, 1,
           "Eclipse Depth   Orbital Phase   Orbital Model   Squint Angle      "
           "AutoTracking");
  ...
  do {
    ...
    if (sat_ele >= 0.0 && antfd != -1) {
      ...
      if ((oldel != iel || oldaz != iaz) ||
          (once_per_second && newtime > lasttime)) {
    ...
    if (sat_ele >= 0.0 && geostationary == 0 && decayed == 0 &&
        daynum > lostime) {
      lostime = FindLOS2();
    ...
    else if (sat_ele < 0.0 && geostationary == 0 && decayed == 0 &&
             aoshappens == 1 && daynum > aoslos) {
...
void MultiTrack() {
  ...
  double aos[24], aos2[24], temptime, nextcalctime = 0.0, los[24], aoslos[24];
  ...
  printw(" Satellite  Az   El %s  %s  Range  | Satellite  Az   El %s  %s  "
         "Range   ",
  ...
  do {
    for (z = 0; z < 24; z++) {
      ...
      if (Decayed(indx, 0.0)) {
        attrset(COLOR_PAIR(2));
        mvprintw(y + 6, x, "%-10s---------- Decayed ---------",
                 Abbreviate(sat[indx].name, 9));
    ...
    if (daynum > nextcalctime) {
      ...
      for (x = 0, y = 0, z = -1; x < 21 && y != 3; x++) {
        if (ok2predict[satindex[x]] && aos2[x] != 0.0) {
          mvprintw(y + 21, 19, "%10s on %s UTC",
                   Abbreviate(sat[(int)satindex[x]].name, 9),
                   Daynum2String(aos2[x]));
...
void Illumination() {
  ...
  if (xterm)
    fprintf(stderr,
            "\033]0;PREDICT: %s's Solar Illumination Calendar For %s\007",
            qth.callsign, sat[indx].name);
  ...
  do {
    ...
    datestring[11] = 0;
    sprintf(string1, "      %s    %4d    %6.2f%c", datestring, 1440 - eclipses,
            sunpercent, 37);
...
void NewUser() {
  ...
  mvprintw(12, 2,
           "WELCOME to PREDICT!  Since you are a new user to the program, "
           "default\n");
  printw("  orbital data and ground station location information was copied "
         "into\n");
  printw("  your home directory to get you going.  Please select option [G] "
         "from\n");
  printw("  PREDICT's main menu to edit your ground station information, and "
         "update\n");
...
int QuickFind(string, outputfile) char *string, *outputfile;
{
  int x, y, z, step = 1;
  ...
  for (z = 0; z < 24; z++) {
    if ((strcmp(sat[z].name, satname) == 0) ||
        (atol(satname) == sat[z].catnum)) {
      start = atol(startstr);
      ...
      if ((start >= now - 31557600) && (start <= now + 31557600) && end == 0) {
        ...
        if (Decayed(indx, daynum) == 0)
          fprintf(fd, "%ld %s %4d %4d %4d %4d %4d %6ld %6ld %c\n", start,
                  Daynum2String(daynum), iel, iaz, ma256, isplat, isplong, irk,
                  rv, findsun);
        break;
      }
        ...
        for (count = start; count <= end; count += step) {
          ...
          if (Decayed(indx, daynum) == 0)
            fprintf(fd, "%ld %s %4d %4d %4d %4d %4d %6ld %6ld %c\n", count,
                    Daynum2String(daynum), iel, iaz, ma256, isplat, isplong,
                    irk, rv, findsun);
        }
        break;
      }
...
int QuickPredict(string, outputfile) char *string, *outputfile;
{
  int x, y, z, lastel = 0;
  long start, now;
        ...
        if (AosHappens(indx) && Geostationary(indx) == 0 &&
          ...
          while (iel >= 0) {
            fprintf(fd, "%.0f %s %4d %4d %4d %4d %4d %6ld %6ld %c %f\n",
                    ...
                    iel, iaz, ma256, isplat, isplong, irk, rv, findsun,
                    doppler100);
            lastel = iel;
            daynum += cos((sat_ele - 1.0) * deg2rad) * sqrt(sat_alt) / 25000.0;
            Calc();
...
int QuickDoppler100(string, outputfile) char *string, *outputfile;
{

  /* Do a quick predict of the doppler for non-geo sattelites, returns UTC epoch
     seconds, UTC time and doppler normalized to 100MHz for every 5 seconds of
     satellite-pass as a CSV*/
        ...
        if (AosHappens(indx) && Geostationary(indx) == 0 &&
          ...
          while (iel >= 0) {
            ...
            fprintf(fd, "%.0f,%s,%f\n", floor(86400.0 * (3651.0 + daynum)),
                    Daynum2String(daynum), doppler100);
            lastel = iel;
...
int main(argc, argv) char argc, *argv[];
{
  int x, y, z, key = 0;
  char updatefile[80], quickfind = 0, quickpredict = 0, quickstring[40],
                       outputfile[42], quickdoppler100 = 0, tle_cli[50],
                       qth_cli[50], interactive = 0;
