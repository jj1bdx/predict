...
char speak;
{
  ...
  int ans, oldaz = 0, oldel = 0, length, xponder = 0, polarity = 0, tshift,
           bshift, spaces, namelength;
  char approaching = 0, command[80], comsat, aos_alarm = 0, geostationary = 0,
       aoshappens = 0, decayed = 0, alstr[80], eclipse_alarm = 0,
       visibility = 0, old_visibility = 0, awake = 0;
  ...
  do {
    ...
    mvprintw(0, 0,
             ...
             "                ");
    mvprintw(1, 0,
             "                     PREDICT Real-Time Satellite Tracking        "
             "                ");
    ...
    */
    ...
    if (sat_sun_status) {
      ...
      if (sat_ele >= 0.0) {
        if ((sun_ele <= -12.0) ||
            (sun_ele <= -8.0 && sat[indx].catnum == 25544))
          visibility_array[indx] = 'V';
      }
    } else
      visibility_array[indx] = 'N';
    ...
    if (sat_ele >= 0.0) {
      ...
      if (speak == 'T' && soundcard) {
        ...
        if ((CurrentDaynum() - oldtime) > (0.00003 * sqrt(sat_alt))) {
          ...
          sprintf(command, "%svocalizer/vocalizer %d %d %c %c &", predictpath,
                  iaz, iel, approaching, visibility);
...
void MultiTrack() {
  ...
  mvprintw(4, 0,
           " Satellite  Az   El %s  %s  Range  | Satellite  Az   El %s  %s  "
           "Range   ",
